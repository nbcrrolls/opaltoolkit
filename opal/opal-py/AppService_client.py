...
class AppServicePortTypeSoapBindingSOAP:
    ...
    # op: getSystemInfo
    def getSystemInfo(self, request, **kw):
        if isinstance(request, getSystemInfoRequest) is False:
            raise TypeError, "%s incorrect request type" % (request.__class__)
        # no input wsaction
        self.binding.Send(None, None, request, soapaction="http://nbcr.sdsc.edu/opal/getSystemInfo", **kw)
        # no output wsaction
        response = self.binding.Receive(getSystemInfoResponse.typecode)
        return response
